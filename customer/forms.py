...
class CustomerForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "birth_date": forms.DateInput(
                attrs={"class": "form-control", "type": "date", "min": "1940-01-01"}
            ),
            "email": forms.EmailInput(
                attrs={
                    ...
                    "pattern": r"^[^\s@]+@[^\s@]+\.[^\s@]+$",
                    "title": "Insira um e-mail válido (ex: usuario@dominio.com)",
                }
            ),
            "phone": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "(00) 00000-0000"}
            ),
            "baby_due_date": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "size_preferences": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Ex: P, M, G"}
            ),
            "note": forms.Textarea(
                attrs={
                    "class": "form-control",
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Define datas dinâmicas para validação
        hoje = date.today()
        max_baby_due_date = hoje + timedelta(weeks=50)

        # birth_date: máximo é a data atual
        self.fields["birth_date"].widget.attrs["max"] = hoje.isoformat()

        # baby_due_date: máximo 50 semanas à frente
        self.fields["baby_due_date"].widget.attrs["max"] = max_baby_due_date.isoformat()
...
class AddressForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "state": forms.Select(
                choices=ESTADOS_BRASILEIROS, attrs={"class": "form-control"}
            ),
            "city": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Nome da cidade"}
            ),
            "neighborhood": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Nome do bairro"}
            ),
            "street": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Nome da rua"}
            ),
